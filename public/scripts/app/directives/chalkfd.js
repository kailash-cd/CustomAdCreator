...
directive('chalkfd', ['$document','$log','Util','$timeout', function($document,$log,Util,$timeout) {
    ...
    var controller = ['$scope', function ($scope) {
        $log.debug("this is mp dir controller");
    }];
    var template = '<style>.bigButton1{background-size: 138px 19px; margin: 10px; border: none; background-color: #5bc0de;' +
        ' color:white; display: inline; cursor: pointer; text-align: center; width: 138px;height: 30px;}</style>' +
                ...
                return {
                    ...
                    post: function(scope, el,attrs) {
                        ...
                        function getCoordinates() {
                            ...
                            var temp1 = [];
                            var temp = [];
                            newTargetData.freeDrawingData = allShape;
                            newTargetData.ShapeidServer = Shapeid;
                            newTargetData.AllShapeLocServer = AllShapeLoc;
                            newTargetData.freeDrawingZoom = map.getZoom();
                            if (!Util.isVoid(newTargetData.freeDrawingData)) {
                                for (var t = 0; t < newTargetData.freeDrawingData.length; t++) {
                                    $log.debug(newTargetData.freeDrawingData[t].type)
                                    if (newTargetData.freeDrawingData[t].type === "ractangle") {
                                        temp1 = [];
                                        temp1.push(newTargetData.freeDrawingData[t].east);
                                        temp1.push(newTargetData.freeDrawingData[t].north);
                                        temp.push(temp1);
                                        temp1 = [];
                                        temp1.push(newTargetData.freeDrawingData[t].west);
                                        temp1.push(newTargetData.freeDrawingData[t].north);
                                        temp.push(temp1);
                                        temp1 = [];
                                        temp1.push(newTargetData.freeDrawingData[t].west);
                                        temp1.push(newTargetData.freeDrawingData[t].south);
                                        temp.push(temp1);
                                        temp1 = [];
                                        temp1.push(newTargetData.freeDrawingData[t].east);
                                        temp1.push(newTargetData.freeDrawingData[t].south);
                                        ...
                                        for (var u = 0; u < newTargetData.freeDrawingData[t].positions.length; u++) {
                                            //$log.debug("positions are",newTargetData.freeDrawingData[t].positions[u]);
                                            var pos = [];
                                            for (var c = newTargetData.freeDrawingData[t].positions[u].length - 1; c >= 0; c--) {
                                                pos.push(newTargetData.freeDrawingData[t].positions[u][c]);
                                            }
                                            temp.push(pos);
                                        }
                                    }
                                    newTargetData.geoBoundary[t] = {outerPolygon: temp, innerPolygons: null};
                                    temp = [];
                                }
                            }
                            $log.debug("Exporting newTargetData ....",newTargetData);
                        }
