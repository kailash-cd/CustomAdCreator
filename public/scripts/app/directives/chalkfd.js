...
directive('chalkfd', ['$document','$log','Util','$timeout', function($document,$log,Util,$timeout) {
    ...
    var template = '<style>.bigButton1{background-size: 138px 19px; margin: 10px; border: none; background-color: #E81;' +
        ...
        +   '<style>#errormessage { z-index: 5; background-color: #fff;color:red; padding: 5px;border: none; text-align: center;}</style><div id="errormessage"></div>'
        +   '<center style="display:block;"><button style="margin-left: 6px" class="bigButton1" id="getcood">Generate Boundries' +
            ...
            return {
                ...
                post: function(scope, el,attrs) {
                    ...
                    function getUpdatedCirclePoint(circle) {
                        ...
                        Shapeid = Shapeid + 1;
                        scope.allShape[AllShapeLoc] = setShapeData;
                    ...
                    function clearSelectionAndAvails() {
                        if (selectedShape) {
                            ...
                            // fetchAvails(scope.allShape,"freedrawing")
                        }
                    }
                    ...
                    function getCoordinates() {
                        if(scope.allShape.length == 0) {
                            document.getElementById("errormessage").innerHTML = "Please draw some shapes";
                            return;
                        }
                        ...
                        var temp = [];
                        scope.$apply(function () {
                            scope.geoFencingData.freeDrawingData = scope.allShape;
                    ...
                    function addListenersOnPolygon(polygon, i, shapeType) {
                        ...
                        if (shapeType === 'polygon') {
                            google.maps.event.addListener(polygon.getPath(), 'insert_at', function () {
                                ...
                                scope.allShape[i].centerPoint = tempGeoBoundary;
                            ...
                            google.maps.event.addListener(polygon.getPath(), 'set_at', function () {
                                ...
                                scope.allShape[i].centerPoint = tempGeoBoundary;
                                scope.allShape[i].positions = tempGeoBoundary;
                                selectedPolygonIndex = -1;
                                polygon.setEditable(false);
                            });
                            ...
                            google.maps.event.addListener(polygon, 'bounds_changed', function () {
                                ...
                                var south = sw.lat();
                                var west = sw.lng();
                                scope.allShape[i].north = north;
                                scope.allShape[i].south = south;
                                scope.allShape[i].east = east;
                            ...
                            google.maps.event.addListener(polygon, 'radius_changed', function () {
                                ...
                                scope.allShape[i].radius = polygon.getRadius();
                                scope.allShape[i].positions = circumference;
                            ...
                            google.maps.event.addListener(polygon, 'dragend', function () {
                                ...
                                var pointA = new google.maps.LatLng(polygon.getCenter().lat(), polygon.getCenter().lng());
                                var circumference = getCircleBoundary(pointA, polygon.getRadius());
                                var polyshape = scope.allShape[i];
                                scope.allShape[i].centerPoint = [polygon.getCenter().lat(), polygon.getCenter().lng()];
                                scope.allShape[i].positions = circumference;
                            });
                    ...
                    function plotSavedPoly() {
                        $log.debug("@plotSavedPoly .. ", "allShpes :: ",scope.allShape)
                        document.getElementById("errormessage").innerHTML = "";
                        var savedPolygon = [];
                        PolygonArray = [];
                        drawPolygon = [];
                        if (scope.allShape.length > 0)
                        {
                            for (var i = 0; i < scope.allShape.length; i++) {
                                var polygon_plan_0 = [];
                                var tempArray = scope.allShape[i].centerPoint;
                                if (scope.allShape[i].type === 'polygon') {
                                ...
                                else if (scope.allShape[i].type === 'circle') {
                                    var center = {"lat": scope.allShape[i].centerPoint[0], "lng": scope.allShape[i].centerPoint[1]};
                                    drawPolygon.push(new google.maps.Circle({
                                        ...
                                        clickable: true,
                                        zIndex: 1,
                                        radius: scope.allShape[i].radius
                                    }));
                                    var radius = scope.allShape[i].radius / 1000;
                                    radius = radius.toFixed(2);
                                } else if (scope.allShape[i].type === 'ractangle') {
                                    var bounds = {
                                        north: scope.allShape[i].north,
                                        south: scope.allShape[i].south,
                                        east: scope.allShape[i].east,
                                        west: scope.allShape[i].west
                                    };
                                ...
                                addListenersOnPolygon(drawPolygon[drawPolygon.length - 1], i, scope.allShape[i].type);
                            }
                        }
                    };
                    function deleteSelectedShape() {
                        if(!Util.isVoid(trackPolygon)){
                            ...
                            if (polyshape.type === "polygon" && polyshape.positions.length <= 2) {
                                ...
                                scope.allShape.splice(trackPolygon, 1);
                                AllShapeLoc = AllShapeLoc - 1;
                                deleteSavedPoly();
                                plotSavedPoly();
